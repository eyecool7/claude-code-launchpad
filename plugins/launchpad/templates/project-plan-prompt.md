...
이 문서는 claude-code-launchpad 스킬의 입력으로 사용된다.
...
> **실제 검색과 설치는 refine 단계(`/launchpad:refine`)에서 Claude Code가 수행한다.**
...
아래 3가지 지표를 평가하고, 결과를 `AGENT_TIER` 값과 함께 **판정 근거**로 기록한다:

| 지표 | Tier 1 | Tier 2 | Tier 3 |
|------|--------|--------|--------|
| P0+P1 기능 수 | 3개 이하 | 4~7개 | 8개 이상 |
| 독립 작업 블록 | 없음 (순차 의존) | 2개 이상 (병렬 가능) | 4개 이상 (팀 분업 필요) |
| Phase 수 | 2개 이하 | 3~4개 | 5개 이상 |

- 3개 지표 중 **2개 이상** 해당하는 Tier로 판정
- 경계선(예: 기능 4개 + 독립 블록 1개)이면 **낮은 Tier 선택** (과도한 구조 방지)

**판정 결과 기록 형식:**
```
AGENT_TIER: {1/2/3}
판정 근거: 기능 {N}개, 독립 블록 {N}개, Phase {N}개 → Tier {N}
```

> 사용자가 Tier를 변경하고 싶으면 setup 미리보기에서 "Tier 1로 해줘" 등으로 오버라이드 가능.
...
- 만들 것 목록 — 각 항목에 아래를 포함:
  ...
  - **테스트 전략**: TDD 적용 여부 (로직 중심 → ✅ TDD, UI/설정 → ❌)
