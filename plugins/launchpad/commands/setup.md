...
계획서 작성은 `/launchpad:plan` → `/launchpad:refine` 순서로 진행한다.
...
1. **세션 1 — 기획:** `/launchpad:plan` + `/launchpad:refine`으로 계획서 작성 → 파일 저장 → 세션 종료
2. **세션 2 — 구현:** 새 세션 시작 → `/launchpad:setup` → 이 커맨드 실행
...
🤖 작업 방식: Tier {1/2/3} — {순차 작업 / 서브에이전트 / 에이전트 팀}
   판정 근거: 기능 {N}개, 독립 블록 {N}개, Phase {N}개
{Tier별 한 줄 설명}
(Tier 2/3이면 커스텀 에이전트 목록도 표시)
💡 Tier 변경: "Tier 1로 해줘" 등으로 오버라이드 가능
...
- **작업 방식**: 계획서 4번의 AGENT_TIER 값 + **판정 근거**(기능 수, 독립 블록 수, Phase 수)를 읽어 표시. Tier 2/3이면 커스텀 에이전트 이름도 나열. 판정 근거가 계획서에 없으면 섹션 1(기능 수) + 섹션 6(Phase 수)에서 직접 계산.
- **충돌 경고**: Step 1의 CONFLICT_WARNINGS가 있으면 표시. 없으면 생략.

**승인 흐름:**
- 사용자가 승인 → Step 4로 진행
- 사용자가 수정 요청 → 반영 후 미리보기 재출력 → 재승인
  - 예: "이 스킬은 빼줘" → 해당 스킬 제거
  - 예: "Tier 1로 해줘" → Tier 변경 + 커스텀 에이전트 제거
  - 예: "Tier 2로 올려줘" → Tier 변경 + 에이전트 추가 안내
- 사용자가 거부 → 세팅 중단
...
`/clear`로 컨텍스트를 비우고 `/launchpad:build`를 실행하면 계획서 기반 개발이 시작됩니다.
...
🤖 작업 방식: Tier 1 — 순차 작업
   판정 근거: 기능 3개, 독립 블록 0개, Phase 3개
한 번에 하나의 기능씩 완성 후 다음으로 이동.
💡 Tier 변경: "Tier 2로 올려줘" 등으로 오버라이드 가능
