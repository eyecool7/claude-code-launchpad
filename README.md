...
바이브 코더를 위한 Claude Code Launchpad
...
> **Optimized** = 각 프로젝트의 스택, 규모, 요구사항에 맞춰 최적화된 세팅
...
5단계 명령어를 순서대로 따라가면, 프로젝트 설정과 구축이 완성됩니다.

0. **`/project-launch:guide`** — 온보딩: 전체 워크플로우 안내 (처음 사용 시)
...
7개 보안 훅이 개발 전 과정을 자동으로 감시합니다. API 키 유출 감지(secret-guard), 위험 명령 차단(command-guard), 민감 파일 수정 알림(security-trigger), 파괴적 SQL 차단(db-guard), 커밋 전 검증(pre-commit-check)이 자동 작동합니다. `/project-launch:security`로 CWE Top 25 기반 보안 스캔도 수시로 실행할 수 있습니다. setup 과정에서는 검증 스크립트가 스택 충돌, 환경 설정 누락, 보안 허점을 사전에 잡아냅니다.
...
`/project-launch:guide` 명령어 실행 → 전체 워크플로우와 생성되는 파일 안내 출력
...
| 구성요소 | 설명 |
|----------|------|
| 커맨드 (6개) | `/project-launch:guide`, `plan`, `refine`, `setup`, `build`, `security` |
