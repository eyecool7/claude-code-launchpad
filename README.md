...
고도화된 계획 수립, 계획 기반 맞춤 구축, 철통 보안까지 한번에 잡고 싶은 분을 위한 워크플로우입니다.

- 계획서의 중요성은 아는데, 어디서부터 시작해야 하는지 막막하신 분
- 내 프로젝트에 기술 스펙을 정하기 어려운 분
  - 순차 작업, 서브에이전트, 팀모드 중 뭘 골라야 할지 모르는 분
  - 어떤 스킬과 MCP가 내 프로젝트에 필요한지 감이 안 오는 분
  - 프로젝트에 상관없이 보안 설정은 양보 없이 철통으로 지키고 싶은 분
  - CLAUDE.md를 58줄로 쓸지 500줄로 쓸지 고민되는 분
- 계획서와 기술 스펙을 기반으로 맞춤 자동 세팅을 원하시는 분
- 단계별 구축 가이드를 통해 전 과정 맥락을 유지하고 싶은 분

---

## 어떻게 작동하나요?

5단계 명령어를 Optimized PBS 순서대로 따라가면, 프로젝트 맞춤 계획부터 구축·보안까지 완성됩니다.
...
/plan → /refine → /setup → /build 명령어를 순서대로 실행하여 Optimized PBS 워크플로우를 완성할 수 있습니다. setup 단계에서 CLAUDE.md, rules, skills, agents, commands, hooks, settings까지 24개 파일이 계획서 기반으로 자동 생성됩니다. CLAUDE.md의 길이, 보안 설정의 수준, 에이전트 구성까지 모두 계획서가 결정하므로 프로젝트 맞춤 컨텍스트가 만들어지고 기획의도에 충실한 결과물이 구축됩니다.
